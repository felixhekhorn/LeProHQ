...
public:
     ...
     */
    Grid(const Lumi& lumi, const list<Order>& orders, const list<double>& bins, const KeyVal& key_val) {
        ...
        this->raw = pineappl_grid_new(lumi.raw, this->n_orders, raw_orders, this->n_bins, raw_bins, key_val.raw);
        // clean up
        delete[] raw_orders;
        delete[] raw_bins;
    }
     ...
     * @return prediction for each bin
     */
    vector<double> convolute_with_one(int32_t pdg_id, LHAPDF::PDF *pdf, double xi_ren = 1.0, double xi_fac = 1.0 ) const {
        list<bool> order_mask(this->n_orders, true);
        list<bool> lumi_mask(this->n_lumis, true);
     ...
     * @return prediction for each bin
     */
    vector<double> convolute_with_one(int32_t pdg_id, LHAPDF::PDF *pdf, const list<bool>& order_mask, const list<bool>& lumi_mask, double xi_ren = 1.0, double xi_fac = 1.0) const {
        ...
        for (auto it = lumi_mask.cbegin(); it != lumi_mask.cend(); ++it, ++j)
            raw_lumi_mask[j] = *it;}
        vector<double> results(this->n_bins - 1);
