"""Next-to-leading Order Bethe-Heitler Coefficient Function."""
import numba as nb
import numpy as np
...
def cq1hv(proj, cc, xi, eta):
    ...
    if proj == "F2":
        return (
            bmsn.c2ps2am0_aq2(z) * lnxi**2
            + bmsn.c2ps2am0_aq(z) * lnxi
            + bmsn.c2ps2am0_a0(z)
        ) / n
    if proj == "FL":
        return (bmsn.clps2am0_aq(z) * lnxi + bmsn.clps2am0_a0(z)) / n
...
def cq1(proj, cc, xi, eta, path=None):
    ...
    return raw_c(
        proj, cc, xi, eta, path, "cq1", cq1t, cq1hv, np.log(1e0), np.log(1.5e3)
    )
