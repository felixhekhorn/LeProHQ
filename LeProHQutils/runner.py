...
from LeProHQpp import InclusiveLeptoProduction, FullyDiffLeptoProduction, DynamicScaleFactors
...
def global_setter(o,p):
    ...
    if "projection" in p: o.setProjection(p["projection"])
    if "nlf" in p: o.setNumberOfLightFlavours(p["nlf"])
    if "Q2" in p: o.setQ2(p["Q2"])
    if "m2" in p: o.setM2(p["m2"])
    # partonic setter
    if "partonicEta" in p: o.setPartonicEta(p["partonicEta"])
    if "partonicS" in p: o.setPartonicS(p["partonicS"])
    # hadronic setter
    if "pdf" in p: o.setPdf(*p["pdf"])
    if "muR2" in p: o.setMuR2(DynamicScaleFactors(*p["muR2"]))
    if "muF2" in p: o.setMuF2(DynamicScaleFactors(*p["muF2"]))
    if "mu2" in p:  o.setMu2 (DynamicScaleFactors(*p["mu2"]))
    if "lambdaQCD" in p: o.setLambdaQCD(p["lambdaQCD"])
    if "alphaSByLHAPDF" in p: o.setAlphaSByLHAPDF(*p["alphaSByLHAPDF"])
    if "xBjorken" in p:  o.setXBjorken(p["xBjorken"])
    if "hadronicS" in p: o.setHadronicS(p["hadronicS"])
    if "flags" in p:
        for k in p["flags"]:
            setattr(o.flags(),k,p["flags"][k])
    # leptonic setter
    if "alphaEM" in p: o.setAlphaEM(p["alphaEM"])
    if "polarizeBeams" in p: o.setPolarizeBeams(p["polarizeBeams"])
    if "leptonicS" in p: o.setLeptonicS(p["leptonicS"])
    if "Q2min" in p: o.setQ2min(p["Q2min"])
    if "Q2minByHVQDIS" in p: o.setQ2minByHVQDIS(p["Q2minByHVQDIS"])
...
def run_inclusive(p):
    ...
    # setters
    global_setter(o,p)
    if "Delta" in p: o.setDelta(p["Delta"])
